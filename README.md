...
2. **실시간 기기 오프라인 감지 및 자동 화면 비움 (Telemetry Timeout)**
   - 라즈베리파이 센서 기기의 작동이 중단되거나 연결이 유실되어 **12초간 데이터 수신이 없을 경우**, 백엔드 메모리 캐시를 즉시 비우고 브라우저로 리셋 신호를 송출합니다.
...
```mermaid
graph TD
    Device[YOLOv8 IoT Sensor Node] -- 1. Telemetry State Update (REST POST) --> BE[Spring Boot Backend]
...
#### A. 카메라 비디오 스트리밍 서버 가동
라즈베리파이 터미널에서 Flask 패키지를 설치한 후 아래 코드를 구동하여 실시간 MJPEG 영상 송출 웹서버를 시작합니다.
```bash
...
#### B. 가상환경 진입 및 인식 프로그램 실행
라즈베리파이의 가상환경에 진입한 후, 좌석 인식 설정 프로그램 또는 최종 통합 실시간 제어 프로그램을 구동합니다.
```bash
# 1. 가상환경 진입
...
# 3. 최종 통합 프로그램 실행 (실시간 인식 및 백엔드 텔레메트리 전송)
python3 main.py
```
